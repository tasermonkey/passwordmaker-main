...
window._hpwmbklhash123456_v = {
	...
	generatePassword:function() {
		...
		if (this.characters.length < 2) {
			alert('Character Set is less than 2 characters!');
			return;
		}
		mpw = document.getElementById(this.id+'mpw1').value;
		if (this.mphash) {
			// Compare hash to Master Password
		} else {
			if (mpw != document.getElementById(this.id+'mpw2').value) {
				alert('Master Passwords do not match!');
				document.getElementById(this.id+'mpw1').focus();
				return;
			}
		}
		data = this.usetext + this.username + this.modifier;
	...
	getText: function() {
		var temp, domainSegments, displayMe = '', displayMeTemp;
		temp = location.href.match(/^([^:]+:\/\/)([^\/]+)(.*)$/);
		if (!temp) {
			temp = ['','','',''];
		}
		domainSegments = temp[2].split('.');
		/*while (domainSegments.length < 3) {
			domainSegments.unshift('');
		}*/
		displayMeTemp = this.protocol?temp[1]:'';
