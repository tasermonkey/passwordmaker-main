			...
			for (int i = 0; i < rdfDes.size(); ++i) {
				...
				*/
				if (des.attributeNS(rdfNS, "about") == "http://passwordmaker.mozdev.org/globalSettings") {
					...
					sTemp = des.attributeNS(pwmNS, "maskMasterPassword");
					vTemp = QVariant(sTemp);
					vTemp.convert(QVariant::Bool);
					setSetting("maskGenerated", vTemp, 1);

					sTemp = des.attributeNS(pwmNS, "hideMasterPasswordField");
					vTemp = QVariant(sTemp);
					vTemp.convert(QVariant::Bool);
					setSetting("hideMaster", vTemp, 1);

					sTemp = des.attributeNS(pwmNS, "confirmMPW");
					vTemp = QVariant(sTemp);
					vTemp.convert(QVariant::Bool);
					setSetting("confirmMaster", vTemp, 1);

					sTemp = des.attributeNS(pwmNS, "autoClearClipboard");
					vTemp = QVariant(sTemp);
					vTemp.convert(QVariant::Bool);
					setSetting("autoClearClipboard", vTemp, 1);

					sTemp = des.attributeNS(pwmNS, "autoClearClipboardSeconds");
					vTemp = QVariant(sTemp);
					vTemp.convert(QVariant::Int);
					setSetting("autoClearClipboardSeconds", vTemp, 1);
					/*
					// RDF - XML
					// Not used yet
...
bool PWMSettings::exportSettings(QString xmlPath, bool rdf) {
	...
	if (rdf) {
		...
		while (query.next()) {
			if (query.value(0).toString() == "maskGenerated") {
				settings.setAttribute(pwmNS + "maskMasterPassword", query.value(1).toString());
			}
			if (query.value(0).toString() == "hideMaster") {
				settings.setAttribute(pwmNS + "hideMasterPasswordField", query.value(1).toString());
			}
			if (query.value(0).toString() == "confirmMaster") {
				settings.setAttribute(pwmNS + "confirmMPW", query.value(1).toString());
			}
			if (query.value(0).toString() == "autoClearClipboard") {
				settings.setAttribute(pwmNS + "autoClearClipboard", query.value(1).toString());
			}
			if (query.value(0).toString() == "autoClearClipboardSeconds") {
				settings.setAttribute(pwmNS + "autoClearClipboardSeconds", query.value(1).toString());
			}
			/*
				// Fields used by RDF for master password hash
				useMultipleMasterPasswords - If multiple Master Password hashes are being used. If account support is not used, only used to know if we need to look at default setting or not
				mpwSalt2 - Salt for MPW hash (2 because of logic bug in a beta of 1.7 Firefox edition, and to allow hash function to work). Used as the HMAC-SHA256 key
				mpwHash2 - Hash of MPW
			*/
